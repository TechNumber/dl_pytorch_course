...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=3072, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.act = act_layer()
        self.fc2 = nn.Linear(hidden_features, out_features if out_features else in_features)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = nn.functional.softmax((q @ k.transpose(-1, -2)) * self.scale, dim=-1)
        x = self.attn_drop(x)  # TODO: поставить после qkv
        # x = x.transpose(-2, -3).reshape(B, -1, self.head_dim * self.num_heads)
        x = (x @ v).transpose(-2, -3).flatten(start_dim=-2)
...
class ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
                 ...
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, *args, **kwargs):
        super().__init__()
        ...
        self.pos_encoding = nn.Parameter(
            get_positional_encoding(self.patch_embeddings.num_patches + 1, embed_dim),
            requires_grad=False)  # TODO: сравнить с включённым градиентом и со случайной инициализацией
        ...
        # Classifier
        # TODO: Убрать слой активации
        # TODO: заменить на sequential для быстрой отладки
